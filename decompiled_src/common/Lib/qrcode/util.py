#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib\qrcode/util.o
import re
import math
import six
...
MODE_8BIT_BYTE = 4
MODE_KANJI = 8
MODE_SIZE_SMALL = {MODE_NUMBER: 10,
 MODE_ALPHA_NUM: 9,
 MODE_8BIT_BYTE: 8,
 MODE_KANJI: 8}
MODE_SIZE_MEDIUM = {MODE_NUMBER: 12,
 MODE_ALPHA_NUM: 11,
 MODE_8BIT_BYTE: 16,
 MODE_KANJI: 10}
MODE_SIZE_LARGE = {MODE_NUMBER: 14,
 MODE_ALPHA_NUM: 13,
 MODE_8BIT_BYTE: 16,
 MODE_KANJI: 12}
...
BIT_LIMIT_TABLE = [ [0] + [ 8 * sum(map(_data_count, base.rs_blocks(version, error_correction))) for version in xrange(1, 41) ] for error_correction in xrange(4) ]
...
def _lost_point_level3(modules, modules_count):
    ...
    for row in xrange(modules_count):
        ...
        for col in modules_range_short:
            if this_row[col] and not this_row[col + 1] and this_row[col + 2] and this_row[col + 3] and this_row[col + 4] and not this_row[col + 5] and this_row[col + 6]:
                lost_point += 40

    for col in xrange(modules_count):
        for row in modules_range_short:
            if modules[row][col] and not modules[row + 1][col] and modules[row + 2][col] and modules[row + 3][col] and modules[row + 4][col] and not modules[row + 5][col] and modules[row + 6][col]:
                lost_point += 40
...
def optimal_data_chunks(data, minimum = 4):
    ...
    alpha_pattern = re.compile(six.b('[') + re.escape(ALPHA_NUM) + six.b(']') + re_repeat)
