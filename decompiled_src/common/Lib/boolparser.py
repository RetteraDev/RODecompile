#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib/boolparser.o
...
class Tokenizer:
    ...
    def nextTokenTypeIsOperator(self):
        ...
        return t == TokenType.GT or t == TokenType.GTE or t == TokenType.LT or t == TokenType.LTE or t == TokenType.EQ or t == TokenType.NEQ
...
class BooleanParser:
    ...
    def parseTerminal(self):
        if self.tokenizer.hasNext():
            ...
            if tokenType == TokenType.NUM:
                ...
                return n
            if tokenType == TokenType.STR or tokenType == TokenType.VAR:
                n = TreeNode(tokenType)
    ...
    def evaluateRecursive(self, treeNode, variable_dict):
        ...
        if treeNode.tokenType == TokenType.GT:
            return left > right
        if treeNode.tokenType == TokenType.GTE:
            return left >= right
        if treeNode.tokenType == TokenType.LT:
            return left < right
        if treeNode.tokenType == TokenType.LTE:
            return left <= right
        if treeNode.tokenType == TokenType.EQ:
            return left == right
        if treeNode.tokenType == TokenType.NEQ:
            return left != right
        if treeNode.tokenType == TokenType.AND:
            return left and right
        if treeNode.tokenType == TokenType.OR:
