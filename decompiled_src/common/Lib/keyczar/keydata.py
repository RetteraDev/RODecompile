#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib\keyczar/keydata.o
"""
...
class KeyMetadata(object):
    ...
    def Read(json_string):
        ...
        kmd = KeyMetadata(meta['name'], keyinfo.GetPurpose(meta['purpose']), keyinfo.GetType(meta['type']), meta['encrypted'])
...
class KeyVersion(object):
    ...
    def Read(version):
        ...
        @rtype: L{KeyVersion}
        """
        return KeyVersion(version['versionNumber'], keyinfo.GetStatus(version['status']), version['exportable'])
