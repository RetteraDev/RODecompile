#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib\keyczar/keys.o
...
class Key(object):
    ...
    def __eq__(self, other):
        return self.type == other.type and self.size == other.size and self.key_string == other.key_string
    ...
    size = property(lambda self: self.__size, __SetSize, doc='The size of the key in bits.')
    key_string = property(__GetKeyString, doc='The key as a Base64 string.')
    key_bytes = property(lambda self: util.Decode(self.key_string), doc='The key as bytes.')
...
class AesKey(SymmetricKey):
    ...
    def __init__(self, key_string, hmac_key, size = keyinfo.AES.default_size, mode = keyinfo.CBC):
    ...
    def _Hash(self):
        fullhash = util.Hash(util.IntToBytes(len(self.key_bytes)), self.key_bytes, self.hmac_key.key_bytes)
        return util.Encode(fullhash[:keyczar.KEY_HASH_SIZE])
    ...
    def Read(key):
        ...
        aes = json.loads(key)
        hmac = aes['hmacKey']
        return AesKey(aes['aesKeyString'], HmacKey(hmac['hmacKeyString'], hmac['size']), aes['size'], keyinfo.GetMode(aes['mode']))
    ...
    def Encrypt(self, data):
        ...
        sig_bytes = self.hmac_key.Sign(msg_bytes)
    ...
    def Decrypt(self, input_bytes):
        ...
        """
        data_bytes = input_bytes[keyczar.HEADER_SIZE:]
        if len(data_bytes) < self.block_size + util.HLEN:
            raise errors.ShortCiphertextError(len(data_bytes))
        iv_bytes = data_bytes[:self.block_size]
        ciph_bytes = data_bytes[self.block_size:-util.HLEN]
        sig_bytes = data_bytes[-util.HLEN:]
...
class DsaPrivateKey(PrivateKey):
    ...
    def __init__(self, params, pub, key, size = keyinfo.DSA_PRIV.default_size):
...
class RsaPrivateKey(PrivateKey):
    ...
    def __Decode(self, encoded_message, label = ''):
        if len(label) >= 2305843009213693952L:
        ...
        k = int(math.floor(math.log(self.key.n, 256)) + 1)
        diff_len = k - len(encoded_message)
        if diff_len > 1:
            encoded_message = ' ' * (diff_len - 1) + encoded_message
        ...
        expected_label_hash = util.Hash(label)
        if label_hash != expected_label_hash:
            raise errors.KeyczarError('OAEP Decoding Error - hash is invalid')
        delimited_message = datablock[util.HLEN:].lstrip(' ')
        if delimited_message[0] != '':
...
class DsaPublicKey(PublicKey):
    ...
    def _Hash(self):
        fullhash = util.PrefixHash(util.TrimBytes(self._params['p']), util.TrimBytes(self._params['q']), util.TrimBytes(self._params['g']), util.TrimBytes(self._params['y']))
        return util.Encode(fullhash[:keyczar.KEY_HASH_SIZE])
...
class RsaPublicKey(PublicKey):
    ...
    def __Encode(self, msg, label = ''):
        ...
        pad_octets = k - len(msg) - 2 * util.HLEN - 2
        if pad_octets < 0:
            raise errors.KeyczarError('Message is too long: %d' % len(msg))
        datablock = label_hash + ' ' * pad_octets + '' + msg
        ...
        return ' ' + masked_seed + masked_datablock
    ...
    def _Hash(self):
        fullhash = util.PrefixHash(util.TrimBytes(self._params['modulus']), util.TrimBytes(self._params['publicExponent']))
        return util.Encode(fullhash[:keyczar.KEY_HASH_SIZE])
    ...
    def Read(key):
        ...
        pubkey = RSA.construct((util.BytesToLong(params['modulus']), util.BytesToLong(params['publicExponent'])))
    ...
    def Encrypt(self, data):
        ...
        ciph_bytes = self.key.encrypt(data, None)[0]
