#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib\pyasn1\type/univ.o
import string
import types
import operator
...
class Integer(base.AbstractSimpleAsn1Item):
    ...
    def __init__(self, value = None, tagSet = None, subtypeSpec = None, namedValues = None):
    ...
    def prettyIn(self, value):
        ...
        try:
            ...
            try:
                ...
                raise error.PyAsn1Error("Can\'t coerce %s into integer" % value)
    ...
    def clone(self, value = None, tagSet = None, subtypeSpec = None, namedValues = None):
    ...
    def subtype(self, value = None, implicitTag = None, explicitTag = None, subtypeSpec = None, namedValues = None):
        if value is None:
            value = self._value
    ...
    def prettyIn(self, value):
        ...
        if value[0] == "\'":
            if value[-2:] == "\'B":
                ...
                raise error.PyAsn1Error('Bad bitstring value notation %s' % value)
        else:
            for i in string.split(value, ','):
                i = self.__namedValues.getValue(i)
                if i is None:
                    raise error.PyAsn1Error("Unknown identifier \'%s\'" % i)
                if i >= len(r):
        ...
        for element in filter(None, string.split(value, '.')):
            try:
                ...
                try:
                    ...
                    raise error.PyAsn1Error('Malformed Object ID %s at %s: %s' % (str(value), self.__class__.__name__, why))
        ...
        while idx < l:
            ...
            if c is not None:
                if isinstance(c, base.AbstractConstructedAsn1Item):
                    myClone.setComponentByPosition(idx, c.clone(cloneValueFlag=cloneValueFlag))
                else:
                    myClone.setComponentByPosition(idx, c.clone())
            idx = idx + 1

    def _verifyComponent(self, idx, value):
        if self._componentType is not None and not self._componentType.isSuperTypeOf(value):
...
class SequenceAndSetBase(base.AbstractConstructedAsn1Item):
    ...
    def _verifyComponent(self, idx, value):
        ...
        if componentType:
            ...
            if not t.isSuperTypeOf(value):
                raise error.PyAsn1Error('Component type error %s vs %s' % (repr(t), repr(value)))
    ...
    def setComponentByName(self, name, value = None):
        return self.setComponentByPosition(self._componentType.getPositionByName(name), value)
    ...
    def setDefaultComponents(self):
        ...
        while idx:
            ...
            if self._componentType[idx].isDefaulted:
                ...
                if self.getComponentByPosition(idx) is None:
                    raise error.PyAsn1Error('Uninitialized component #%s at %s' % (idx, repr(self)))
...
class Set(SequenceAndSetBase):
    ...
    def getComponentByType(self, tagSet, innerFlag = 0):
        c = self.getComponentByPosition(self._componentType.getPositionByType(tagSet))
...
class Choice(Set):
    ...
    def verifySizeSpec(self):
        self._sizeSpec(' ' * int(self.getComponent() is not None))
    ...
    def _cloneComponentValues(self, myClone, cloneValueFlag):
        try:
            ...
            if isinstance(c, base.AbstractConstructedAsn1Item):
                myClone.setComponentByType(tagSet, c.clone(cloneValueFlag=cloneValueFlag))
            else:
