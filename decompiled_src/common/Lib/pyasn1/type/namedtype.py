#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib\pyasn1\type/namedtype.o
...
class NamedType:
    ...
    def __repr__(self):
        return '%s(%s, %s)' % (self.__class__.__name__, repr(self.__name), repr(self.__type))
                ...
                if uniq:
                    for k in typeMap.keys():
                        if self.__typeMap.has_key(k):
                            raise error.PyAsn1Error('Duplicate type %s in map %s' % (k, self.__typeMap))
