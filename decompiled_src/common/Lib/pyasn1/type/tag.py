#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib\pyasn1\type/tag.o
try:
...
class Tag:
    ...
    def __repr__(self):
        return '%s(tagClass=%s, tagFormat=%s, tagId=%s)' % ((self.__class__.__name__,) + self.__tag)
    ...
    def __ne__(self, other):
        return self is not other and self.__hashedUniqTag != other
    ...
    def __and__(self, (tagClass, tagFormat, tagId)):
        return self.__class__(self.__tag & tagClass, self.__tag & tagFormat, self.__tag & tagId)

    def __or__(self, (tagClass, tagFormat, tagId)):
        return self.__class__(self.__tag[0] | tagClass, self.__tag[1] | tagFormat, self.__tag[2] | tagId)
...
class TagSet:
    ...
    def __repr__(self):
        return '%s(%s)' % (self.__class__.__name__, join(map(lambda x: repr(x), self.__superTags), ', '))

    def __add__(self, superTag):
        return apply(self.__class__, (self.__baseTag,) + self.__superTags + (superTag,))

    def __radd__(self, superTag):
        return apply(self.__class__, (self.__baseTag, superTag) + self.__superTags)
    ...
    def __getitem__(self, idx):
        if type(idx) == SliceType:
            return apply(self.__class__, (self.__baseTag,) + getslice(self.__superTags, idx.start, idx.stop))
    ...
    def __ne__(self, other):
        return self is not other and self.__hashedSuperTags != other
