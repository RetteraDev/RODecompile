#Embedded file name: I:/bag/tmp/tw2/res/entities\common\Lib/decorator.o
"""
    ...
    def __init__(self, func = None, name = None, signature = None, defaults = None, doc = None, module = None, funcdict = None):
        if func:
            ...
            if inspect.isfunction(func):
                self.signature = inspect.formatargspec(formatvalue=(lambda val: ''), *inspect.getargspec(func))[1:-1]
...
def decorator(caller, func = None):
    ...
    if func is None:
        ...
        src = 'def %s(%s): return _call_(caller, %s)' % (caller.__name__, first_arg, first_arg)
        return fun.make(src, dict(caller=caller, _call_=decorator), undecorated=caller)
    else:
...
def new_wrapper(wrapper, model):
    ...
    assert '_wrapper_' not in infodict['argnames'], '\"_wrapper_\" is a reserved argument name!'
