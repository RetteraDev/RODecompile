#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\common/itemToolTipUtils.o
    ...
    if item.isGuanYin():
        if BigWorld.component == 'client':
            ...
            if info['canEquipSuperSkill']:
                skillInfo = {}
                if item.checkGuanYinSuperSkill():
                    bookInfo = GBD.data.get(item.guanYinSuperBookId, {})
                    if bookInfo:
                        pskillId = bookInfo.get('pskillId', [])
                        if len(pskillId) > 0:
                            pskillId = pskillId[0]
