#Embedded file name: /WORKSPACE/data/entities/common/lib/yaml/emitter.o
...
class Emitter(object):
    ...
    def analyze_scalar(self, scalar):
        ...
        followed_by_whitespace = len(scalar) == 1 or scalar[1] in '  	\r\n  '
        ...
        while index < len(scalar):
            ...
            if ch in '\n  ':
                line_breaks = True
            if not (ch == '\n' or ' ' <= ch <= '~'):
                if (ch == '' or ' ' <= ch <= '퟿' or '' <= ch <= '�' or not has_ucs4 or '𐀀' <= ch < '􏿿') and ch != '﻿':
            ...
            followed_by_whitespace = index + 1 >= len(scalar) or scalar[index + 1] in '  	\r\n  '
    ...
    def write_double_quoted(self, text, split = True):
        ...
        while end <= len(text):
            ...
            if end < len(text):
                ch = text[end]
            if ch is None or ch in '\"\\  ﻿' or not (' ' <= ch <= '~' or self.allow_unicode and (' ' <= ch <= '퟿' or '' <= ch <= '�')):
                if start < end:
                    data = text[start:end]
                    self.column += len(data)
                    if self.encoding:
