#Embedded file name: I:/bag/tmp/tw2/res/entities\client\helpers/objgraph.o
"""
...
def find_ref_chain(obj, predicate, max_depth = 20, extra_ignore = ()):
    ...
    .. versionadded:: 1.7
    """
    return find_chain(obj, predicate, gc.get_referents, max_depth=max_depth, extra_ignore=extra_ignore)[::-1]
...
def find_backref_chain(obj, predicate, max_depth = 20, extra_ignore = ()):
    ...
    """
    return find_chain(obj, predicate, gc.get_referrers, max_depth=max_depth, extra_ignore=extra_ignore)


def show_backrefs(objs, max_depth = 3, extra_ignore = (), filter = None, too_many = 10, highlight = None, filename = None, extra_info = None, refcounts = False):
    ...
    """
    show_graph(objs, max_depth=max_depth, extra_ignore=extra_ignore, filter=filter, too_many=too_many, highlight=highlight, edge_func=gc.get_referrers, swap_source_target=False, filename=filename, extra_info=extra_info, refcounts=refcounts)


def show_refs(objs, max_depth = 3, extra_ignore = (), filter = None, too_many = 10, highlight = None, filename = None, extra_info = None, refcounts = False):
    ...
    """
    show_graph(objs, max_depth=max_depth, extra_ignore=extra_ignore, filter=filter, too_many=too_many, highlight=highlight, edge_func=gc.get_referents, swap_source_target=True, filename=filename, extra_info=extra_info, refcounts=refcounts)
...
def show_graph(objs, edge_func, swap_source_target, max_depth = 3, extra_ignore = (), filter = None, too_many = 10, highlight = None, filename = None, extra_info = None, refcounts = False):
    ...
    while queue:
        ...
        del neighbours
        if skipped > 0:
            h, s, v = gradient((0, 1, 1), (0, 1, 0.3), tdepth + 1, max_depth)
            if swap_source_target:
    ...
    if not filename and program_in_path('xdot'):
        ...
        if filename and filename.endswith('.png'):
            ...
            f = os.fdopen(fd, 'wb')
        dot = subprocess.Popen(['dot', '-Tpng', dot_filename], stdout=f, close_fds=False)
        dot.wait()
        f.close()
