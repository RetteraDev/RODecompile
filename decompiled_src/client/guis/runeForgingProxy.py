#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/runeForgingProxy.o
...
class RuneForgingProxy(SlotDataProxy):

    def __init__(self, uiAdapter):
        super(RuneForgingProxy, self).__init__(uiAdapter)
        ...
        self.mediator = None
        self.type = 'runeForging'
        self.bindType = 'runeForging'
        ...
        self.npcId = 0
        self.itemMap = {}
        uiAdapter.registerEscFunc(uiConst.WIDGET_RUNE_FORGING, self.hide)

    def _registerMediator(self, widgetId, mediator):
        if widgetId == uiConst.WIDGET_RUNE_FORGING:
            self.mediator = mediator

    def clearWidget(self):
        self.mediator = None
        gameglobal.rds.ui.unLoadWidget(uiConst.WIDGET_RUNE_FORGING)
    ...
    def show(self, npcId):
        if not self.mediator:
            self.npcId = npcId
            gameglobal.rds.ui.loadWidget(uiConst.WIDGET_RUNE_FORGING)
    ...
    def _getKey(self, bar, slot):
        return 'runeForging%d.slot%d' % (bar, slot)
        ...
        if npc:
            ...
            if posList:
                if self.source == uiConst.RUNE_SOURCE_INV:
                    npc.cell.reforgingRuneInInv(self.invPage, self.invPos, gametypes.RUNE_QIFU_OP_TYPE_ACTIVATE, self.qiFuLv, posList)
                else:
                    npc.cell.reforgingRuneInRuneBoard(self.runePage, self.runePart, gametypes.RUNE_QIFU_OP_TYPE_ACTIVATE, self.qiFuLv, posList)
    ...
    def addForgingItem(self, item):
        if item.id in self.itemMap.values():
            return
        for i in range(5):
            if i not in self.itemMap:
                self.addItem(item, uiConst.RUNE_FORGING_ITEM, i)
                return
        ...
        if self.mediator and item.id in self.itemMap.values():
            for id in self.itemMap:
                if item.id == self.itemMap[id]:
                    self.addItem(item, uiConst.RUNE_FORGING_ITEM, id)

    def addItem(self, item, page, pos):
        if item is not None:
            if page == uiConst.RUNE_FORGING_ITEM:
                self.itemMap[pos] = item.id
                self.refreshConfirmBtn()
            if page == uiConst.RUNE_FORGING_EQUIP and not pos:
    ...
    def removeItem(self, page, pos):
        if page == uiConst.RUNE_FORGING_EQUIP and not pos:
            ...
            self.refreshRuneEquip()
        key = self._getKey(page, pos)
        if page == uiConst.RUNE_FORGING_ITEM and pos in self.itemMap:
    ...
    def onGetToolTip(self, *arg):
        key = arg[3][0].GetString()
        bar, slot = self.getSlotID(key)
        if bar == uiConst.RUNE_FORGING_ITEM:
    ...
    def getEffectText(self, isPreview = False):
        ...
        if i:
            ...
            if uiConst.RUNE_FORGING_LOW_LV in rData.get('qiFuLvList', []):
                ...
                elif isPreview and self.qiFuLv == uiConst.RUNE_FORGING_LOW_LV:
                    runeText += gameStrings.TEXT_RUNEFORGINGPROXY_216
                else:
            ...
            if uiConst.RUNE_FORGING_LHIGH_LV in rData.get('qiFuLvList', []):
                ...
                elif isPreview and self.qiFuLv == uiConst.RUNE_FORGING_LHIGH_LV:
                    runeText += gameStrings.TEXT_RUNEFORGINGPROXY_216
                else:
                    runeText += gameStrings.TEXT_RUNEFORGINGPROXY_218
            runeText += '</font>'
    ...
    def refreshRuneEquip(self):
        ...
        desc = ''
        if i:
            desc = RQD.data.get((uiConst.RUNE_FORGING_LOW_LV, gametypes.RUNE_QIFU_OP_TYPE_ACTIVATE), {}).get('desc', '')
    ...
    def refreshConfirmBtn(self):
        if self.mediator:
            i = self.getRuneEquip()
            if i:
                canRuneQiFuLvList = i.getCanRuneQiFuLv()
                if i.isRune() and canRuneQiFuLvList:
                    if uiConst.RUNE_FORGING_LOW_LV in canRuneQiFuLvList and self.checkQiFuLv(uiConst.RUNE_FORGING_LOW_LV):
                        self.setQiFuLv(uiConst.RUNE_FORGING_LOW_LV)
                        return
                    if uiConst.RUNE_FORGING_LHIGH_LV in canRuneQiFuLvList and self.checkQiFuLv(uiConst.RUNE_FORGING_LHIGH_LV):
                        self.setQiFuLv(uiConst.RUNE_FORGING_LHIGH_LV)
                        return
            self.mediator.Invoke('refreshConfirmBtn', GfxValue(False))
            self.removeItem(uiConst.RUNE_FORGING_EQUIP, 1)
            self.refreshPreview('')

    def checkQiFuLv(self, qiFuLv):
        p = BigWorld.player()
        qiFuData = RQD.data.get((qiFuLv, gametypes.RUNE_QIFU_OP_TYPE_ACTIVATE), {}).get('qiFuData', [])
        if qiFuData:
    ...
    @ui.uiEvent(uiConst.WIDGET_RUNE_FORGING, events.EVENT_INVENTORY_ITEM_CLICKED)
    def onInventoryRightClick(self, event):
        ...
        if i == None:
            ...
            self.setInventoryItem(nPage, nItem, uiConst.RUNE_FORGING_EQUIP, 0)
            return
    ...
    def setInventoryItem(self, nPageSrc, nItemSrc, nPageDes, nItemDes):
        ...
        if not srcItem:
            return
        if srcItem.isRunForging() and nPageDes == uiConst.RUNE_FORGING_ITEM:
            self.addItem(srcItem, nPageDes, nItemDes)
        elif srcItem.isRune() and nPageDes == uiConst.RUNE_FORGING_EQUIP and srcItem.canRuneQiFu() and not nItemDes:
