#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/chickenFoodOtherProxy.o
import BigWorld
import gameglobal
import gamelog
import time
...
class ChickenFoodOtherProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(ChickenFoodOtherProxy, self).__init__(uiAdapter)
        self.widget = None
        self.cfFactory = chickenFoodFactory.getInstance()

    def _registerASWidget(self, widgetId, widget):
        if widgetId == uiConst.WIDGET_CHICKEN_FOOD_OTHER:
            self.widget = widget
            self.initUI()

    def show(self):
        gameglobal.rds.ui.loadWidget(uiConst.WIDGET_CHICKEN_FOOD_OTHER)

    def clearWidget(self):
        self.widget = None
        self.reset()
        gameglobal.rds.ui.unLoadWidget(uiConst.WIDGET_CHICKEN_FOOD_OTHER)
