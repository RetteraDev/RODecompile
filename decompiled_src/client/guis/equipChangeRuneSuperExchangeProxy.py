#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/equipChangeRuneSuperExchangeProxy.o
import BigWorld
import math
import gameglobal
import uiConst
...
class EquipChangeRuneSuperExchangeProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(EquipChangeRuneSuperExchangeProxy, self).__init__(uiAdapter)
        self.widget = None
    ...
    def initUI(self):
        ...
        self.widget.right.selected.selected3.visible = False
        self.widget.right.exchangeBtn.addEventListener(events.BUTTON_CLICK, self.handleExchangeBtnClick, False, 0, True)
        self.widget.helpIcon.helpKey = SCD.data.get('runeSuperExchangeHelpKey', 1)
        self.widget.noItemHint.getChildAt(0).text = gameStrings.EQUIP_CHANGE_RUNE_NO_ITEM_DESC.get('superExchange', '')
        ASUtils.setHitTestDisable(self.widget.right.needCash.cash, True)
        self.widget.right.exchangeBtn.label = gameStrings.EQUIP_CHANGE_RUNE_SUPER_EXCHANGE

    def handleExchangeBtnClick(self, *args):
        rightData = self.getRightData()
        selectedItem = self.uiAdapter.equipChangeRuneFeed.getItemByPos(*self.selectedPos)
        itemIdList = rightData['itemIdList']
        itemId = itemIdList[self.rightIndex] if self.rightIndex < len(itemIdList) else 0
        transType = const.HIEROGRAM_TRANS_TYPE_ADVANCE
        ...
        confirmTxt = uiUtils.getTextFromGMD(GMDD.data.RUNE_SUPER_EXCHANGE_CONFIRM, 'GMDD.data.RUNE_SUPER_EXCHANGE_CONFIRM%s,%s')
        p = BigWorld.player()
        isBindItem = False
        if item.Item.isBindItem(selectedItem.id):
            isBindItem = True
        else:
            consumeItemId, _ = rightData.get('consuemData', {}).get('advanceItems', (0, 0))[0]
            if p.inv.countItemBind(consumeItemId, True):
                isBindItem = True
        if isBindItem:
            itemName = p.getRuneData(itemId, 'name', '')
        else:
            itemName = p.getRuneData(item.Item.parentId(itemId), 'name', '')
        confirmTxt = confirmTxt % (p.getRuneOwnProp(itemId), itemName)
    ...
    def getRuneList(self):
        ...
        if self.selectedBag == uiConst.BAG_EQUIP_RUNE:
            for pos, item in hieroCrystals.iteritems():
                if item and p.getRuneData(item.id, 'lv', 0) and item.isNewHieroCrystal() >= minLv and not p.getRuneData(item.id, 'benyuanType', None):
            ...
            for pageIdx, page in enumerate(BigWorld.player().inv.pages):
                for pos, item in enumerate(page):
                    if item and item.isNewHieroCrystal() and p.getRuneData(item.id, 'lv', 0) >= minLv and not p.getRuneData(item.id, 'benyuanType', None):
    ...
    def getRightData(self):
        ...
        self.widget.right.propDesc.text = gameStrings.EQUIP_CHANGE_RUNE_SUPER_DESC
        return rightData
    ...
    def refreshConsume(self):
        ...
        itemIdList = rightData['itemIdList']
        consumeItemId, consuemCnt = consumeData['advanceItems'][0]
