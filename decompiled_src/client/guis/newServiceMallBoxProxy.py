#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/newServiceMallBoxProxy.o
...
class NewServiceMallBoxProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(NewServiceMallBoxProxy, self).__init__(uiAdapter)
        self.widget = None
        self.reset()
    ...
    def initPanel(self, widget):
        self.widget = widget
        activityId, activityData = self.getActivityData()
        if not activityId:
            return
        self.activityId = activityId
        self.displayMode = activityData.get('showTimeType', gametypes.PREFERENTIAL_ACTIVITY_SHOW_TIME_TYPE_START_END)
        self.delTimer()
        self.initUI()
    ...
    def getActivityData(self):
        ...
        for activityId in preferentialActivityList:
            ...
            if not activityData.get('isShowNewServiceActivities', 0):
                continue
            return (activityId, activityData)
    ...
    def initUI(self):
        ...
        for txtDesc, itemsLines in itemList:
            for items in itemsLines:
                ...
                if not items:
                    continue
                itemRenderMc = self.widget.getInstByClsName('NewServiceMallBox_Item')
    ...
    def canOpenTab(self):
        if not commNewServerActivity.isNewServerActivityOpen(const.NEW_SERVICE_MALL_BOX):
            return False
        visible = False
        preferentialActivityList = SCD.data.get('preferentialActivityList', ())
        for activityId in preferentialActivityList:
            activityData = PADD.data.get(activityId, {})
            if not activityData.get('isShowNewServiceActivities', False):
                continue
