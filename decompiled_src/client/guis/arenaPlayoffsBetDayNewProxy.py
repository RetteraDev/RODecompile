#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/arenaPlayoffsBetDayNewProxy.o
...
class ArenaPlayoffsBetDayNewProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(ArenaPlayoffsBetDayNewProxy, self).__init__(uiAdapter)
        self.widget = None
        self.lvKey = ''
        self.betId = 0
        self.teamList = []
    ...
    def initUI(self):
        ...
        self.widget.mainMc.scrollWndList.itemRenderer = 'ArenaPlayoffsBetDayNew_ScrollWndItem'
        self.widget.mainMc.scrollWndList.lableFunction = self.itemFunction
        self.widget.mainMc.scrollWndList.itemHeight = 76
    ...
    def genTestInfo(self):
        self.uiAdapter.arenaPlayoffsBet.teamsDict[self.lvKey] = {'info': {111: {'teamName': 'test1',
                        'hostId': 34343,
                        'teamArenaScore': 100},
                  112: {'teamName': 'test2',
                        'hostId': 34343,
                        'teamArenaScore': 200}}}
        if not self.uiAdapter.arenaPlayoffsBet.candidateDict.has_key(self.lvKey):
            self.uiAdapter.arenaPlayoffsBet.candidateDict[self.lvKey] = {}
        self.uiAdapter.arenaPlayoffsBet.candidateDict[self.lvKey][gametypes.ARENA_PLAYOFFS_BET_TYPE_DUEL, self.betId] = {'data': [(111, 112)]}
    ...
    def updateTeamItem(self, itemMc):
        ...
        if checkIdx != -1:
            itemMc.confirmBtn.enabled = True
        else:
            itemMc.confirmBtn.enabled = False
    ...
    def itemFunction(self, *args):
        ...
        itemMc.costAmountIcon.bonusType = 'wudao'
        itemMc.successText.visible = False
        itemMc.numStepper.count = 1
        itemMc.numStepper.minCount = 1
        itemMc.numStepper.maxCount = DCD.data.get('ARENA_PLAYOFFS_BET_MULTIPLE_LIMIT', 0)
        itemMc.numStepper.enableMouseWheel = False
        itemMc.numStepper.addEventListener(events.EVENT_COUNT_CHANGE, self.handleCountChange, False, 0, True)
        itemMc.confirmBtn.addEventListener(events.MOUSE_CLICK, self.handleClickConfirmBtn, False, 0, True)
        itemMc.costAmount.text = format(DCD.data.get('ARENA_PLAYOFFS_MIN_CASH_LIMIT', 1), ',')
    ...
    def handleClickCheckBox(self, *args):
        ...
        self.updateHistory()
        self.updateTeamItem(itemMc.parent)

    def handleClickConfirmBtn(self, *args):
        e = ASObject(args[3][0])
        idx = e.currentTarget.parent.data.idx
        ansIdx = self.ansList[idx]
        teamInfo = self.teamList[idx]
        numStepper = e.currentTarget.parent.numStepper
        info = {}
        info['lvKey'] = self.lvKey
        info['bType'] = gametypes.ARENA_PLAYOFFS_BET_TYPE_DUEL
        info['betId'] = self.betId
        info['multiple'] = numStepper.count
        info['costAmount'] = info['multiple'] * DCD.data.get('ARENA_PLAYOFFS_MIN_CASH_LIMIT', 1)
        info['team1NUIDs'] = [teamInfo['cId'][0]]
        info['team2NUIDs'] = [teamInfo['cId'][1]]
        betScoreList = uiConst.ARENA_PLAYOFFS_BET_DAY_SCORE_LIST
        info['scoreList1'] = [betScoreList[ansIdx][0]]
        info['scoreList2'] = [betScoreList[ansIdx][1]]
    ...
    def handleCountChange(self, *args):
        e = ASObject(args[3][0])
        costAmountMc = e.currentTarget.parent.costAmount
        costAmount = e.currentTarget.count * DCD.data.get('ARENA_PLAYOFFS_MIN_CASH_LIMIT', 1)
        costAmountMc.text = format(costAmount, ',')
