#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/guildSawmillProxy.o
...
class GuildSawmillProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(GuildSawmillProxy, self).__init__(uiAdapter)
        self.modelMap = {'clickResident': self.onClickResident,
         'clickChange': self.onClickChange}
        self.mediator = None
        self.markerId = 0
        uiAdapter.registerEscFunc(uiConst.WIDGET_GUILD_SAWMILL, self.hide)

    def _registerMediator(self, widgetId, mediator):
        if widgetId == uiConst.WIDGET_GUILD_SAWMILL:
    ...
    def show(self, markerId):
        ...
        if self.mediator:
            ...
            gameglobal.rds.ui.loadWidget(uiConst.WIDGET_GUILD_SAWMILL)

    def clearWidget(self):
        self.mediator = None
        gameglobal.rds.ui.unLoadWidget(uiConst.WIDGET_GUILD_SAWMILL)
    ...
    def refreshInfo(self):
        if self.mediator:
            ...
            info['totalOutput'] = int(marker.getFuncWorkload(guild, ignoreTime=True))
            info['manageRate'] = '%d%%' % int(marker.getMgrFuncWorkEffect(guild) * 100)
            info['scienceRate'] = '%d%%' % int(marker.getTechFuncWorkEffect(guild) * 100)
            info['baseOutput'] = int(marker.getBaseFuncWorkload(guild, ignoreTime=True))
            info['managerField'] = gameStrings.TEXT_GUILDFACTORYPROXY_102 % info['nameTitle']
            if buildValue.buildingId == gametypes.GUILD_BUILDING_LUMBERYARD_ID:
                info['bonusType'] = 'yunmu'
            elif buildValue.buildingId == gametypes.GUILD_BUILDING_MINE_ID:
                info['bonusType'] = 'mojing'
            ...
            jobId = commGuild.getJobIdFromGJRD(self.markerId, gametypes.GUILD_JOB_DIFFICULTY_NORMAL, gametypes.GUILD_JOB_TYPE_FUNC)
            info['normalField'] = GJD.data.get(jobId, {}).get('name', '')
            normalList = []
