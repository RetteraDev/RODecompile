#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/voidLunHuiRewardQueryProxy.o
import BigWorld
...
from data import team_endless_reward_floor_rank_query_reverse_data as TERFRQRD
MAX_REWARDS_NUM = 16

class VoidLunHuiRewardQueryProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(VoidLunHuiRewardQueryProxy, self).__init__(uiAdapter)
        self.widget = None
        self.rank = None
        self.progress = None
        self.reset()
        uiAdapter.registerEscFunc(uiConst.WIDGET_VOID_LUNHUI_REWARDQUERY, self.hide)
    ...
    def _registerASWidget(self, widgetId, widget):
        if widgetId == uiConst.WIDGET_VOID_LUNHUI_REWARDQUERY:
            self.widget = widget
            self.initUI()
            self.refreshInfo()
    ...
    def clearWidget(self):
        self.widget = None
        self.uiAdapter.unLoadWidget(uiConst.WIDGET_VOID_LUNHUI_REWARDQUERY)

    def show(self, rank, progress):
        self.rank = rank
        self.progress = progress
        if not self.widget:
            self.uiAdapter.loadWidget(uiConst.WIDGET_VOID_LUNHUI_REWARDQUERY)
        else:
            self.refreshInfo()

    def initUI(self):
        self.widget.defaultCloseBtn = self.widget.closeBtn
        self.widget.scrollWndList.itemRenderer = 'VoidLunHuiRewardQuery_rewardItem'
    ...
    def itemHeightFunction(self, *args):
        ...
        tReward = TERFRQRD.data.get(self.rank, {}).get(key, {}).get('edsAwdItem', ())
    ...
    def itemFunction(self, *args):
        ...
        key = tuple(itemData.key)
        rewardData = TERFRQRD.data.get(self.rank, {})
