#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/tabAuctionConsignProxy.o
...
class TabAuctionConsignProxy(SlotDataProxy):

    def __init__(self, uiAdapter):
        super(TabAuctionConsignProxy, self).__init__(uiAdapter)
        self.modelMap = {'registerMc': self.onRegisterMc,
         ...
         'initSearchItem': self.onInitSearchItem,
         'openCharge': self.onOpenCharge}
        self.panelMc = None
        self.buyMediator = None
        self.bidMediator = None
        self.bindType = 'tabAuctionConsign'
        self.type = 'tabAuctionConsign'
        self.pageItems = None
        self.curDbId = None
        self.pageSrc = -1
        self.posSrc = -1
        ...
        self.keepTime = 0
        self.consignSearchHistory = TabAuctionConsignSearchHistory()
        uiAdapter.registerEscFunc(uiConst.WIDGET_TAB_CONSIGN_BID, self.closeBidPanel)
        uiAdapter.registerEscFunc(uiConst.WIDGET_TAB_CONSIGN_BUY, self.closeBuyPanel)
    ...
    def onRegisterMc(self, *arg):
        ...
        self.addEvent(events.EVENT_CASH_CHANGED, self.onCoinChanged)
        ...
        if self.searchItemName:
            self.initInfo['searchName'] = self.searchItemName
        return uiUtils.dict2GfxDict(self.initInfo, True)

    def _registerMediator(self, widgetId, mediator):
        if widgetId == uiConst.WIDGET_TAB_CONSIGN_BUY:
            self.buyMediator = mediator
        elif widgetId == uiConst.WIDGET_TAB_CONSIGN_BID:
    ...
    def show(self, npcId = 0, layoutType = uiConst.LAYOUT_DEFAULT, searchItemName = '', seekId = 0):
        if not BigWorld.player().checkMapLimitUI(gametypes.MAP_LIMIT_UI_CONSIGN):
            ...
            if not self.isShow():
                gameglobal.rds.ui.tabAuction.show(0, npcId=npcId, layoutType=layoutType)
            elif self.panelMc != None:
                self.panelMc.Invoke('refreshConsign', GfxValue(gbk2unicode(self.searchItemName)))
            return
    ...
    def showBuyPanel(self):
        if self.buyMediator:
            ...
            gameglobal.rds.ui.loadWidget(uiConst.WIDGET_TAB_CONSIGN_BUY)

    def closeBuyPanel(self):
        self.buyMediator = None
        gameglobal.rds.ui.unLoadWidget(uiConst.WIDGET_TAB_CONSIGN_BUY)

    def showBidPanel(self):
        if self.bidMediator:
            self.bidMediator.Invoke('refreshItemInfo', (self.onGetMinBidPrice(), self.onGetBuyItemInfo()))
        else:
            gameglobal.rds.ui.loadWidget(uiConst.WIDGET_TAB_CONSIGN_BID)

    def closeBidPanel(self):
        self.bidMediator = None
        gameglobal.rds.ui.unLoadWidget(uiConst.WIDGET_TAB_CONSIGN_BID)
    ...
    def setStorageFee(self, ctype):
        ...
        if self.panelMc != None:
            self.panelMc.Invoke('setSellPanelFee', GfxValue(val))
    ...
    def setSortState(self, idx, state):
        if self.panelMc != None:
            self.panelMc.Invoke('setSortState', (GfxValue(idx), GfxValue(state)))

    def addConsignItem(self):
        if self.panelMc != None:
            self.panelMc.Invoke('addConsignItem')
    ...
    def searchItemsByName(self, name, cate = [''], *arg):
        ...
        if name:
            ...
            def searchComplete(searchId, itemIds):
                if not self.panelMc:
                    return
                if self.curConsignType == uiConst.CONSIGN_TYPE_COMMON:
                    if not self.buyTabInfo.searchByName(e, name, school=school, mType=category, sType=subcategory, itemIdArr=itemIds):
                        self.buyTabInfo.clearCache()
                        self.showTab()
                elif self.curConsignType == uiConst.CONSIGN_TYPE_COIN:
                    if not self.buyCoinTabInfo.searchByName(e, name, school=school, mType=category, sType=subcategory, itemIdArr=itemIds):
                        self.buyCoinTabInfo.clearCache()
                        self.showTab()
            ...
            curSearchArgs = {'owner': e,
             ...
             'hasItemName': False,
             'consignType': self.curConsignType}
            self.searchItemId = sHelper.addConsignTask(curSearchArgs, searchComplete)
        else:
            forme = arg[3][1].GetBool() if arg else 0
            minLv = arg[3][2].GetString().strip() if arg else 0
            if not minLv:
            ...
            if self.curTabInfo == self.historyTabInfo:
                ...
                for itemData in items:
                    ...
                    info.append([uiUtils.getItemIconFile64(item.id),
                     ...
                     item.getSpecialPropLevel()])

            self.panelMc.Invoke('setDetailCanvas', (uiUtils.array2GfxAarry(info, True), GfxValue(self.curTabInfo.curPage + 1), GfxValue(self.curTabInfo.totalPages)))
    ...
    def onSellItem(self, *arg):
        ...
        if self.curConsignType == uiConst.CONSIGN_TYPE_COMMON:
            ...
            if utils.needDisableUGC() and (num in UGCForceLimit or bidPrice in UGCForceLimit or fixedPrice in UGCForceLimit):
                BigWorld.player().showGameMsg(GMDD.data.UGC_FORCE_LIMIT_MSG, ())
                return
            self.sellCommonItem(*arg)
        elif self.curConsignType == uiConst.CONSIGN_TYPE_COIN:
            if gameconfigCommon.enableUGCForceLimit() and (num in UGCForceLimit or bidPrice in UGCForceLimit):
                BigWorld.player().showGameMsg(GMDD.data.UGC_FORCE_LIMIT_MSG, ())
                return
    ...
    def onSellItemSuccess(self):
        ...
        if self.panelMc != None:
            self.panelMc.Invoke('clearSellInput')
    ...
    def onNumChange(self, *arg):
        ...
        curItem = self.bindingData.get('tabAuctionConsign1.slot99', None)
        self.consignNum = num
    ...
    def onGetItemNames(self, *arg):
        ...
        if self.curConsignType == uiConst.CONSIGN_TYPE_COMMON:
            data = IND.data.get(uiConst.ITEM_NAME_CONSIGN, {})
            if 'dc' in pinyin:
                names = list(set(data.get(pinyin[0], []) + data.get('z', [])))
            else:
                names = data.get(pinyin[0], [])
        else:
    ...
    def setCancelBtnVisible(self, visible):
        if self.panelMc != None:
            self.panelMc.Invoke('setCancelBtnVisible', GfxValue(visible))
    ...
    def enableCoinConsign(self):
        ...
        if self.panelMc:
            self.panelMc.Invoke('updateCoinConsignEnable', GfxValue(enable))
    ...
    def onInventoryRightClick(self, event):
        gamelog.debug('@zq onInventoryRightClick')
        if self.panelMc:
            event.stop()
            i = event.data['item']
    ...
    def setInventoryItem(self, nPageSrc, nItemSrc, nPageDes, nItemDes):
        ...
        if self.curConsignType == uiConst.CONSIGN_TYPE_COIN and not i.isItemCoinConsign():
            ...
            return
        self.addConsignItem()
        gameglobal.rds.ui.tabAuctionConsign.setItem(nPageDes, nItemDes, i, nPageSrc, nItemSrc)
...
class TabAuctionConsignSearchHistory(SearchHistoryUtils):

    def __init__(self):
        super(TabAuctionConsignSearchHistory, self).__init__('TabAuctionConsign')
        self.maxCount = 20
