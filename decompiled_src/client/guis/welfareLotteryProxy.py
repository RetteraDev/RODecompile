#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/welfareLotteryProxy.o
import BigWorld
import utils
import time
import const
...
class WelfareLotteryProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(WelfareLotteryProxy, self).__init__(uiAdapter)
        self.widget = None
        self.lotteryId = 0
        self.lotteryTime = 0
        self.callback = None
    ...
    def initLottery(self, widget):
        self.widget = ASObject(widget)
        self.initUI()
        self.refreshInfo()

    def unRegistLottery(self):
        self.widget = None
        self.reset()

    def getLotteryLabel(self):
        lotteryId = NSAD.data.get('globalLotteryId', 0)
        lotteryData = LD.data.get(lotteryId, {})
        if lotteryData:
            return lotteryData.get('lotteryTitle', gameStrings.WELFARE_LOTTERY_LABEL)
        return gameStrings.WELFARE_LOTTERY_LABEL
    ...
    def isOpen(self):
        if not gameglobal.rds.configData.get('enableGlobalLottery', False):
            return False
        lotteryId = NSAD.data.get('globalLotteryId', 0)
        if not lotteryId:
            return False
