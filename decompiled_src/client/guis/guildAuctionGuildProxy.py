#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/guildAuctionGuildProxy.o
import BigWorld
...
class GuildAuctionGuildProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(GuildAuctionGuildProxy, self).__init__(uiAdapter)
        self.itemNameList = []
        self.reset()
        self.clearAll()
    ...
    def initUI(self):
        ...
        self.widget.historyBtn.groupName = 'tab'
        self.widget.historyBtn.tabIdx = uiConst.GUILD_AUCTION_SECOND_TAB_HISTORY
        self.widget.historyBtn.addEventListener(events.BUTTON_CLICK, self.handleClickTab, False, 0, True)
        self.widget.timeOutHint.htmlText = uiUtils.getTextFromGMD(GMDD.data.GUILD_AUCTION_GUILD_TIME_OUT_HINT, '')
        buyPanel = self.widget.buyPanel
        buyPanel.scrollWndList.itemRenderer = 'GuildAuctionGuild_BuyItem'
        ...
        for label, filterKey in GCD.data.get('guildAuctionTypeViewList', ()):
            itemMc = self.widget.getInstByClsName('GuildAuctionGuild_TypeItem')
            itemMc.label = label
        ...
        buyPanel.helpIcon.helpKey = GCD.data.get('guildAuctionGuildHelpIconHelpKey', 0)
        ...
        bidPanel.scrollWndList.itemRenderer = 'GuildAuctionGuild_BidItem'
        ...
        historyPanel = self.widget.historyPanel
        historyPanel.scrollWndList.itemRenderer = 'GuildAuctionGuild_HistoryItem'
        historyPanel.scrollWndList.dataArray = []
        historyPanel.scrollWndList.lableFunction = self.historyItemFunction
        historyPanel.scrollWndList.itemHeight = 55

    def queryInfo(self):
        if self.checkAuctionClose():
            return
        BigWorld.player().base.getAllGuildConsignments(self.itemInfolVer)

    def checkAuctionClose(self):
        if not self.stateDict:
            return True
        for val in self.stateDict.itervalues():
            if val != gametypes.GUILD_CONSIGN_STATE_CLOSE:
                return False

        return True

    def updateItemListInfo(self, itemInfoList, itemInfolVer, maxProfit):
        self.itemInfoDict = {}
        for itemInfo in itemInfoList:
            ...
            self.itemInfoDict[itemInfo[0]] = {'itemId': itemId,
             ...
             'bidPrice': gcidData.get('guildBidPrice', 0),
             'fixedPrice': gcidData.get('guildFixPrice', 0),
             'bidMakup': gcidData.get('guildBidMakup', 0),
    ...
    def updateMaxProfit(self, maxProfit):
        self.maxProfit = maxProfit
        self.refreshMaxProfitInfo()
    ...
    def updateOpenPushMsg(self):
        if not gameglobal.rds.configData.get('enableGuildConsign', False) or self.checkAuctionClose() or self.checkIsSoul():
            self.uiAdapter.pushMessage.removePushMsg(uiConst.MESSAGE_TYPE_GUILD_AUCTION_GUILD_OPEN)
        else:
            self.uiAdapter.pushMessage.addPushMsg(uiConst.MESSAGE_TYPE_GUILD_AUCTION_GUILD_OPEN)

    def checkIsSoul(self):
        p = BigWorld.player()
        if p._isSoul() and not gameglobal.rds.configData.get('enableGuildAuctionCrossPush', False):
            return True
        return False

    def updateHistoryInfoList(self, historyInfoList, historyInfoVer):
        self.historyInfoList = sorted(historyInfoList, key=lambda x: x[5], reverse=True)
        self.historyInfoVer = historyInfoVer
    ...
    def selectTab(self, tabIdx):
        ...
        elif tabIdx == uiConst.GUILD_AUCTION_SECOND_TAB_HISTORY:
            self.widget.historyBtn.selected = True
            BigWorld.player().base.getAllGoodsHistory(self.historyInfoVer)
        self.selectedItemUUID = 0
        self.selectedMc = None
    ...
    def refreshInfo(self):
        ...
        if self.currentTabIndex == uiConst.GUILD_AUCTION_SECOND_TAB_BUY:
            ...
            self.refreshBidInfo()
        elif self.currentTabIndex == uiConst.GUILD_AUCTION_SECOND_TAB_HISTORY:
            self.refreshHistoryInfo()
    ...
    def refreshBuyInfo(self):
        ...
        if self.checkAuctionClose():
            ...
            self.widget.historyPanel.visible = False
            return
    ...
    def refreshMaxProfitInfo(self):
        if not self.widget:
            return
        self.widget.buyPanel.maxProfit.text = format(self.maxProfit, ',')
    ...
    def buyItemFunction(self, *args):
        ...
        itemMc.activityName.text = self.getActivityName(itemDetail.get('vendorSource', 0))
    ...
    def getActivityName(self, sourceId):
        guildAuctionSaleSourceDict = GCD.data.get('guildAuctionSaleSourceDict', {})
        sourceName = guildAuctionSaleSourceDict.get(sourceId, '')
        if not sourceName:
            for key in guildAuctionSaleSourceDict.keys():
                if type(key) in (tuple, list) and len(key) == 2:
                    if int(key[0]) <= sourceId <= int(key[1]):
                        sourceName = guildAuctionSaleSourceDict.get(key, '')
                        break
    ...
    def refreshHistoryInfo(self):
        if not self.widget:
            return
        if self.currentTabIndex != uiConst.GUILD_AUCTION_SECOND_TAB_HISTORY:
            return
        historyPanel = self.widget.historyPanel
        self.widget.timeOutHint.visible = False
        self.widget.buyPanel.visible = False
        ...
        historyPanel.scrollWndList.dataArray = self.historyInfoList
        historyPanel.scrollWndList.validateNow()

    def historyItemFunction(self, *args):
        itemData = ASObject(args[3][0])
        itemMc = ASObject(args[3][1])
        itemMc.overMc.visible = False
        itemId = itemData[1]
        ...
        itemMc.itemSlot.setItemSlotData(uiUtils.getGfxItemById(itemId, stateFlag=True))
        itemMc.itemName.htmlText = uiUtils.getItemColorName(itemId)
        if itemData[2] == gametypes.GUILD_CONSIGN_SELLOUT_TYPE_BID:
            itemMc.result.gotoAndStop('bid')
            itemMc.result.price.text = itemData[3]
        elif itemData[2] == gametypes.GUILD_CONSIGN_SELLOUT_TYPE_FIXED:
            itemMc.result.gotoAndStop('fixed')
            itemMc.result.price.text = itemData[3]
        else:
            itemMc.result.gotoAndStop('no')
        itemMc.activityName.text = self.getActivityName(itemData[4])
        itemMc.dealTime.text = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(itemData[5]))
        itemMc.addEventListener(events.MOUSE_ROLL_OVER, self.handleOverItem, False, 0, True)
        itemMc.addEventListener(events.MOUSE_ROLL_OUT, self.handleOutItem, False, 0, True)
    ...
    def handleClickBidItemBtn(self, *args):
        ...
        self.uiAdapter.guildAuctionBuy.show(uiConst.GUILD_AUCTION_BUY_TYPE_GUILD_BID, itemInfo)
    ...
    def handleClickBuyItemBtn(self, *args):
        ...
        itemInfo = {'uuid': self.selectedItemUUID,
         ...
         'vendorSource': itemDetail.get('vendorSource', 0)}
        self.uiAdapter.guildAuctionBuy.show(uiConst.GUILD_AUCTION_BUY_TYPE_GUILD_BUY, itemInfo)
