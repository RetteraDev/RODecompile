#Embedded file name: C:/Users/user/Documents/WORK/roB/build/tmp/tw2/res/entities\client\guis/equipChangeInlayProxy.o
...
class EquipChangeInlayProxy(UIProxy):

    def __init__(self, uiAdapter):
        super(EquipChangeInlayProxy, self).__init__(uiAdapter)
        self.modelMap = {'registerMc': self.onRegisterMc,
         'unRegisterMc': self.onUnRegisterMc,
         'addedToStage': self.onAddToStage}
        self.panelMc = None
        self.resetData()
    ...
    def refreshGemList(self, gemItems):
        self.selectedGemItemMc = None
        self.widget.gemList.scrollList.itemRenderer = 'EquipChangeGem_GemItem'
        ...
        if tabIdx == RES_KIND_EQUIP:
            for i, item in enumerate(p.equipment):
                ...
                if not item.canInlayGem():
                    continue
                itemInfo = self.getLeftItemData(item, tabIdx, const.ITEM_IN_EQUIPMENT)
            ...
            for pg in p.inv.getPageTuple():
                for ps in p.inv.getPosTuple(pg):
                    ...
                    if not item.canInlayGem():
                        continue
                    itemInfo = self.getLeftItemData(item, tabIdx, const.ITEM_IN_BAG)
                    itemInfo['sortIdx'] = uiUtils.getEquipSortIdxByPart(item)
    ...
    def getDetailItemData(self, item):
        ...
        itemInfo['itemName'] = uiUtils.getItemColorNameByItem(item, True, -1, True)
        itemInfo['sortIdx'] = uiUtils.getEquipSortIdxByPart(item)
        itemInfo['yinSlots'] = self.getItemSlotData(uiConst.GEM_TYPE_YIN, item)
        itemInfo['yangSlots'] = self.getItemSlotData(uiConst.GEM_TYPE_YANG, item)
    ...
    def getSrcTypeStr(self):
        str = 'equipChangeInlay%d' % len(self.gemItemCache)
        return str
    ...
    def onGetToolTip(self, srcType):
        index = int(srcType.replace('equipChangeInlay', ''))
