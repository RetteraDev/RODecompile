#Embedded file name: /WORKSPACE/data/entities/client/guis/rankingproxy.o
import time
import BigWorld
...
class RankingProxy(UIProxy):
    ...
    def _registerMediator(self, widgetId, mediator):
        ...
        if widgetId == uiConst.WIDGET_RANK_AWARD:
            self.rewardMed = mediator
            return GfxValue(1)
        if widgetId == uiConst.WIDGET_RANK_AWARD_V2:
            self.rewardMed = mediator
            return GfxValue(2)
    ...
    def _generateTestData(self, proxyID, info):
        ...
        for index in xrange(1, 20):
            obj = {}
            obj['index'] = index
            obj['roleName'] = '角色名称八个字' + str(index)
            ...
            obj['now'] = '%d年%d月%d日' % (t.tm_year, t.tm_mon, t.tm_mday)
            ret['list'].append(obj)
    ...
    def refreshGuildData(self, iType, info, lastWeekRankInfo):
        if self.mediator:
            ...
            p = BigWorld.player()
            if iType == const.PROXY_KEY_TOP_GUILD_PROSPERITY:
                rewardInfo['val'] = '繁荣度: %d' % (p.guild.prosperity if p.guild else 0)
            elif iType == const.PROXY_KEY_TOP_GUILD_MATCH:
                rewardInfo['val'] = '众里寻他积分: %d' % (p.guild.matchScore if p.guild else 0)
            elif iType == const.PROXY_KEY_TOP_GUILD_KINDNESS:
                rewardInfo['val'] = '爱心值: %d' % (p.guild.kindness / 100 if p.guild else 0)
            elif iType == const.PROXY_KEY_TOP_GUILD_FISH_ACTIVITY:
                rewardInfo['val'] = '钓鱼大赛积分: %d' % (p.guild.fishActivityScore if p.guild else 0)
            elif iType == const.PROXY_KEY_TOP_GUILD_CHICKEN_MEAL:
                rewardInfo['val'] = '百凤宴积分: %d' % (p.guild.chickenMealScore if p.guild else 0)
            elif iType == const.PROXY_KEY_TOP_GUILD_MONSTER_CLAN_WAR:
                rewardInfo['val'] = '四方之乱积分: %d' % (p.guild.monsterClanWarScore if p.guild else 0)
            elif iType == const.PROXY_KEY_TOP_GUILD_PRESTIGE:
                rewardInfo['val'] = '威望: %d' % (p.guild.prestige if p.guild else 0)
    ...
    def refreshEquipView(self, info):
        if self.mediator:
            ...
            p = BigWorld.player()
            rewardInfo = {}
            rewardInfo['score'] = '装备评分:' + str(p.equipment.calcAllEquipScore(p.suitsCache))
            rewardInfo['time'] = rankCommonUtils._getDeadlineTime()
    ...
    def _genTeamDetailData(self, data):
        ...
        for i in xrange(len(data)):
            ...
            teamObj['school'] = SD.data[teamData['school']]['name'] if teamData['school'] in self.schoolList else '未知'
